...
/**
 * Benchmark comparing the old (pre-PR#13863) and current writeString implementations in
 * ByteBuffersDataOutput.
 *
 * <p>Each invocation writes ~16MB of string data into a fresh ByteBuffersDataOutput (starting from
 * 1024-byte blocks), exercising the full block growth lifecycle: small blocks at the start,
 * boundary crossings, block expansion, and eventually large blocks where the fast path dominates.
 */
...
public class WriteStringBenchmark {
  ...
  /** Number of strings to write per invocation to reach ~16MB total output. */
  ...
  public void currentImpl(Blackhole bh) {
    // Fresh output each invocation — starts from 1024-byte blocks, grows naturally
  ...
  public void oldImpl(Blackhole bh) throws IOException {
    // Fresh output each invocation — same block growth behavior
    ByteBuffersDataOutput output = new ByteBuffersDataOutput();
    for (int i = 0; i < stringsPerInvocation; i++) {
    ...
    for (int i = 0; i < length && pos < chars.length - 1; i++) {
      ...
      if (roll < 90) {
        ...
        if (pos < chars.length - 1) {
          chars[pos++] = (char) (0xD800 + random.nextInt(0x3FF)); // high surrogate
          chars[pos++] = (char) (0xDC00 + random.nextInt(0x3FF)); // low surrogate
        } else {
          chars[pos++] = (char) (0x4E00 + random.nextInt(0x9FFF - 0x4E00));
        }
      }
