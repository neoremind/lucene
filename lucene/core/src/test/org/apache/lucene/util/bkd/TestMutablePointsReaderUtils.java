...
public class TestMutablePointsReaderUtils extends LuceneTestCase {
  ...
  public void testSortAssumeDocIdIsIncremental() {
  ...
  private void doTestSort(boolean isDocIdIncremental) {
    ...
    BKDConfig config =
        new BKDConfig(
            1, 1, bytesPerDim, BKDConfig.DEFAULT_MAX_POINTS_IN_LEAF_NODE, isDocIdIncremental);
  ...
  private void doTestSortByDim() {
    ...
    Point[] points = createRandomPoints(config, maxDoc, commonPrefixLengths, true);
    DummyPointsReader reader = new DummyPointsReader(points);
